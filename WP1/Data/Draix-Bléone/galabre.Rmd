...
galabre_old <- merge(debit, mes, by = "DateHeure")

galabre_new <- readxl::read_xlsx(path = "Galabre_outlet.xlsx")
...
galabre$commondate <- as.Date(paste0("2000-", format(debit$Date,"%j")), "%Y-%j")
...
by = "year", all = T) %>% 
  merge(galabre %>% subset(!is.na(doc_mg.L)) %>% group_by(year) %>% summarise(nb_doc = n()), by = "year", all = T) %>%
...
ggplot(galabre, aes(x = commondate)) + 
  geom_point(aes(y = mes_g.L), col = "palegreen4", size = 0.5) +
  scale_x_date(labels = function(x) format(x, "%d-%b"), date_minor_breaks = "1 month", name = "")+
  facet_grid(rows = vars(year))+
  labs(x = "", y = "CMES, g/L") +
  theme_minimal()+
...
```
```{r annual-q-mes-new, fig.dim = c(10, 10)}

ggarrange(
ggplot(debit_new, aes(x = commondate)) + 
  geom_point(aes(y = q_m3.s), col = "lightblue", size = 0.5) +
  scale_x_date(labels = function(x) format(x, "%d-%b"), date_minor_breaks = "1 month", name = "")+
  facet_grid(rows = vars(year))+
  labs(x = "", y = "Q, m3/s") +
...
ggplot(crues)+
  geom_point(aes(x=mes_g.L, y = poc_percent, col = "measured"))+ 
...
tail_value_galabre <- crues[order(crues$mes_g.L, decreasing = T),] %>% pull(poc_percent) %>% head(dim(crues)[1]/10) %>% median() # 10% of poc_percent corresponding to the max cmes

# add mes for POC 2011
crues$mes_for_fit <- crues$mes_g.L
crues$tail <- tail_value_galabre

pocfit_galabre <- nls(
  poc_percent ~ a * mes_for_fit^b + tail,
  data = subset(crues, !is.na(poc_percent) & !is.na(mes_for_fit) & poc_percent != 0 & mes_g.L != 0),
  start = list(a = start_a, b = start_b)
)

summary(pocfit_galabre)
crues$poc_nls_fitted <- predict(pocfit_galabre, crues)

formula_pocfit_galabre <- paste("NLS : POC =", round(coef(pocfit_galabre)[1],2), "* MES (mg/L) ^", round(coef(pocfit_galabre)[2],2), "+", round(tail_value_galabre,2))

ggplot(crues)+
  geom_point(aes(x=mes_for_fit, y = poc_percent, col = "measured"))+ 
  geom_point(aes(x=mes_for_fit, y = poc_nls_fitted, col = "fitted"))+ 
  geom_abline(slope = 0, intercept = tail_value_galabre, col = "darkred")+
  annotate(geom = "label", label = formula_pocfit_galabre, x = Inf, y = Inf, hjust = 1.1, vjust = 1.1)+
  scale_color_manual(values = c("darkblue", "lightblue"))+
  labs(x = "MES, mg/L", y = "POC, %")+
  theme_minimal()

plot(resid(pocfit_galabre)~fitted(pocfit_galabre))
