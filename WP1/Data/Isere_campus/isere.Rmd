...
```
In this model, the residuals are heteroskedatic (noise increases with POC%). To solve this, we will try to model the noise based on x. 
...
poc_isere <- isere %>% subset(!is.na(poc_percent)) # subsets poc_percent values in isere

poc_isere$mes_for_fit <- poc_isere$mes_mg.L # rename the MES column
poc_isere$mes_for_fit[which(is.na(poc_isere$mes_for_fit))] <- poc_isere$cmes_g.L[which(is.na(poc_isere$mes_for_fit))]*1e3 # The 2011 POC measurements do not include measured MES. The CMES value from turbidity is assigned instead. 
poc_isere <- poc_isere %>% subset(!is.na(mes_for_fit)) # finally, remove from from new dataset all poc values that do not have any associated MES/CMES value.

poc_isere$poc_prop <- poc_isere$poc_percent / 100 # converts % in proportion between 0 and 1 for modelling. 
nrow_isere <- nrow(isere)
...
## Powerlaw with Claude

```{r fit-poc-mes-gnls}
poc_isere <- filter(isere, !is.na(poc_prop))
...
# there are only 96 fitted values -> gnls silently suppresses some lines
fitted(pocfitg)[which(poc_isere$poc_prop == 0)]
