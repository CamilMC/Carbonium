...
tail_value <- poc_isere[order(poc_isere$cmes_g.L, decreasing = T),] %>% pull(poc_percent) %>% head(dim(poc_isere)[1]/10) %>% median() 
...
pred.boot <- apply(theta_poc, 1, function (cb){
  with(as.list(cb), a*poc_isere$mes_for_fit^b + tail_value)
})
...
for(i in 1:nrow_theta){
   mu_nls[i,] <- with(as.list(theta_poc[i,]), # listing the coefficients allows to have 2 named numerics to be used in the function
                      f(isere$mes_for_fit, a, b))
}

## adds noise
pred_nls <- mu_nls + matrix(rnorm(n = nrow_theta*nrow_isere, mean = 0, sd = sigma_poc), nrow = nrow_theta)
