...
for(i in name_crues){
  ...
  #compute sum poc pour 2015
  # 
  # df2015 <- subset(df, year == 2015 & !is.na(q_m3.s))
  # sum_
...
ggplot(poc_isere)+geom_point(aes(x = cmes_g.L, y = poc_percent, col = name))+
  geom_abline(slope = 0, intercept = tail_value, col = "darkred")

poc_isere$mes_mg.L[which(is.na(poc_isere$mes_mg.L))] <- poc_isere$cmes_g.L[which(is.na(poc_isere$mes_mg.L))]*1e3
poc_isere$tail <- tail_value

ggplot(poc_isere)+geom_point(aes(x=mes_mg.L, y = poc_percent))+ geom_abline(slope = 0, intercept = tail_value, col = "darkred")

poc_isere$poc_hand_fitted <- with(poc_isere, mes_mg.L^-0.25+0.99)
ggplot(poc_isere)+geom_point(aes(x=mes_mg.L, y = poc_hand_fitted))+ geom_abline(slope = 0, intercept = tail_value, col = "darkred")

fit_poc <- glm(data = subset(poc_isere, !is.na(mes_mg.L) & poc_percent != 0), poc_percent ~ mes_mg.L + 0 + offset(log(tail)), family = gaussian(link = "log"))
summary(fit_poc)

poc_isere$poc_fitted <- predict(fit_poc, poc_isere) %>% exp()

ggplot(poc_isere, aes(x=poc_percent, y = poc_fitted))+geom_point()

ggplot(poc_isere)+geom_point(aes(x=mes_mg.L, y = poc_fitted))+ geom_abline(slope = 0, intercept = tail_value, col = "darkred")
```
