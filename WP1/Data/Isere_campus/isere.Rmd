...
```{r fit-poc-mes-forcing-end-value, eval = F}
...
# estimates noise with beta-distribution
y <- poc_isere$poc_percent
is_zero <- y == 0
y_pos <- y[!is_zero]
p <- poc_isere$poc_nls_fitted[!is_zero]

var_pos <- var(y_pos) # variance of positive observation
p_mean <- mean(p) #mean of positive pred

phi_hat <- mean(p_mean*(1-p_mean)/var_pos) - 1
alpha_hat <- p_mean*phi_hat
beta_hat <- (1 - p)*phi_hat

zero_model <- glm(is_zero ~mes_for_fit, family = binomial, data = poc_isere)

p_zero_hat <- predict(zero_model) # useless here ? Not used in confidence interval

# compute se.fit (not supported by "predict" for nls models) - bootstrap
library(nlstools)
boot <- nlsBoot(pocfit, niter = 1000)
theta_poc <- boot$coefboot

pred.boot <- apply(theta_poc, 1, function (cb){ # cb = coefficients boot: the function applys to each couple of a and b
  with(as.list(cb), a*poc_isere$mes_for_fit^b + aspt_value)
})
ci <- apply(pred.boot, 1, quantile, probs = c(0.025, 0.975))
poc_isere$poc_fitted_ci_low <- ci[1,]
poc_isere$poc_fitted_ci_high <- ci[2,]
poc_isere$poc_fitted_boot <- apply(pred.boot, 1, mean) #identique à poc_nls_fitted
...
ggplot(poc_isere)+
  ...
  geom_point(aes(x=mes_for_fit, y = poc_fitted_boot, col = "fitted", shape = "fitted"))+ 
  geom_ribbon(aes(x = mes_for_fit, ymin = poc_fitted_ci_low, ymax = poc_fitted_ci_high), fill = "gray80", alpha = 0.5)+
...
#plot residuals -> check 
plot(abs(resid(pocfit))~fitted(pocfit))

```
In this model, the residuals are heteroskedastic (noise increases with POC%). To solve this, we will try to model the noise based on x. 



### Extension of POC values to all dataset

The uncertainty of MES has to be included already in the prediction. 

```{r all-poc, eval = F}
isere$mes_for_fit <- isere$cmes_g.L * 1e3 
isere$mes_for_fit <- ifelse(isere$mes_for_fit ==0, NA, isere$mes_for_fit)

isere$poc_prop <- isere$poc_percent / 100

nrow_isere <- nrow(isere)
...
mes_mat <- matrix(NA_real_, nrow = 1000, ncol = nrow_isere)

for(i in 1:nrow(isere)){
  mes_mat[,i] <-rnorm(1000, mean = isere$mes_for_fit[i], sd = mes_u) # chaque colonne de la matrice correspond à une observation de MES
}
...
## bootstrapping of the distribution of the mean response

nrow_theta <- nrow(theta_poc)


### version avec matrice de mes
mu_nls <- matrix(NA_real_, nrow = nrow_theta, ncol = nrow_isere)
for(i in 1:nrow_theta){
   mu_nls[i,] <- with(as.list(theta_poc[i,]), # listing the coefficients allows to have 2 named numbers to be used in the function
                      f(mes_mat[i, ], a, b))
}


isere$poc_var_mean <- apply(mu_nls, 2, var) # variance of fit parameters + MES 


## adds noise

### mean-dependant noise
#delta <- coef(lm(log(abs(residuals(pocfit)))~log(fitted(pocfit))))[2] # 1 < delta < 2
#pred_nls <- mu_nls + matrix(rnorm(n = nrow_theta*nrow_isere, mean = 0, sd = sigma_poc*mu_nls^delta), nrow = nrow_theta) #sigma is the standard deviation of the random error term

### adds noise with sigma_logit
#pred_nls_logit <- qlogis(mu_nls) + matrix(rnorm(n = nrow_theta*nrow_isere, mean = 0, sd = sigma_logit), nrow = nrow_theta) #sigma is the standard deviation of the random error term
#pred_nls <- plogis(pred_nls_logit)




### computes variance, y-wise
isere$poc_var_tot <- apply(pred_nls, 2, var) # total variance per prediction, for prediction total uncertainty
...
## insert in isere
isere$poc_pred_upper <- pi[, "upper"]
isere$poc_pred_lower <- pi[, "lower"]

# plot

ggplot(isere, aes(x = mes_for_fit))+
  geom_line(aes(y = poc_pred_lower, ymax = poc_pred_upper), fill = "gray80", alpha = 0.5)+
  geom_line(aes(y = poc_pred_upper), fill = "gray80", alpha = 0.5)+
  geom_point(aes(y = poc_prop, col = "measured"))+
...
### test beta noise chatgpt

```{r test-beta-noise, eval = F}

### Select data for modeling

nrow_isere <- nrow(isere)

### 1- Fit POC~MES with nls (power law with forced asymptotic value)
aspt_value <- poc_isere[order(poc_isere$cmes_g.L, decreasing = T),] %>% pull(poc_prop) %>% head(dim(poc_isere)[1]/10) %>% median() # computes asymptotic value as the median of POC prop of the 10 higher MES
poc_isere$aspt <- aspt_value
...
sigma_poc <- summary(pocfit)$sigma # sigma is the irreducible noise of the nls model

poc_isere$poc_fitted <- predict(pocfit, poc_isere) # predicted values (from original data)
plot(residuals(pocfit)~fitted(pocfit)) # heteroskedatiscity -> model noise with beta distribution
plot(residuals(pocfit)~poc_isere$mes_for_fit) # heteroskedatiscity -> model noise with beta distribution
...
pred.boot <- apply(theta_poc, 1, function (cb){
  with(as.list(cb), a*poc_isere$mes_for_fit^b + aspt_value)
})
ci <- apply(pred.boot, 1, quantile, probs = c(0.025, 0.975))
poc_isere$poc_fitted_ci_low <- ci[1,]
...
### NOISE: estimate beta precision from positive residuals
poc_prop <- poc_isere$poc_prop
poc_fitted <- poc_isere$poc_fitted

is_zero_obs <- poc_prop == 0

poc_prop_pos <- poc_prop[!is_zero_obs]
poc_fitted_pos <- poc_fitted[!is_zero_obs]

## empirical variance of positive results
var_pos <- var(poc_prop_pos - poc_fitted_pos)

## theoretical beta variance = mu(1-mu)/(1+phi) -> approximated phi using mean values
poc_fitted_mean <- mean(poc_fitted_pos)

phi <- (poc_fitted_mean*(1-poc_fitted_mean)/var(poc_prop_pos)) - 1
...
# Uncertainty on MES
isere$cmes_mg.L <- isere$cmes_g.L * 1000
fit_cmes <- lm(mes_mg.L~cmes_mg.L, data = isere)
summary(fit_cmes)

mes_u <- summary(fit_cmes)$coefficients[4]

# matrix of MES in 1000 MC worlds
mes_mat <- matrix(NA_real_, nrow = 1000, ncol = nrow_isere)
...
nrow_theta <- nrow(theta_poc) # from part 1

# matrice de POC intégrant incertitude sur MES et incertitude sur nls coefficients

a_vec <- theta_poc[,"a"]
b_vec <- theta_poc[,"b"]

mu_nls <- sweep(mes_mat^(matrix(b_vec, nrow_theta,nrow_isere)), 1, a_vec, '*') + aspt_value

isere$poc_var_mean <- apply(mu_nls, 2, var) # variance of fit parameters + MES 


# adds noise to mu_nls

### zero probability model: rare so fitted with constant probability
p_zero_const <- mean(poc_isere$poc_prop == 0) #proportion de O dans poc_prop

zero_sim_mat <- matrix(
  rbinom(nrow_theta*nrow_isere, 1, p_zero_const),
  nrow = nrow_theta
)
View(zero_sim_mat) 
...
mu_nls <- pmin(pmax(mu_nls, aspt_value + eps), 1 - eps) # forces the mu_nls that would be 0 or 1 to be slightly over or under, so that a beta noise can be predicted
var_target <- ((mu_nls - aspt_value)^delta) * phi

precision <- mu_nls * (1-mu_nls)/var_target - 1 # total precision parater of the beta distribution
alpha_mat <- mu_nls*precision

beta_mat  <- (1 - mu_nls) * precision

alpha_mat[alpha_mat <= 0] <- eps
beta_mat[beta_mat <= 0] <- eps

# beta noise
beta_sim_mat <- matrix(
  rbeta(nrow_theta * nrow_isere,
        shape1 = as.vector(alpha_mat),
        shape2 = as.vector(beta_mat)),
  nrow = nrow_theta
)

pred_nls <- beta_sim_mat # final prediction including noise
pred_nls[zero_sim_mat == 1] <- 0 # randomly predicts 0 in predicted the 1000 MC worlds of poc_prop, impossible otherwise
...
isere$var_mean <- apply(mu_nls, 2, var)
isere$var_tot <- apply(pred_nls,2,var)

ggplot(isere, aes(x=poc_fitted))+
  geom_point(aes(y = var_mean, col = "var mean"))+
  geom_point(aes(y = var_tot, col = "var tot"))+
  theme_minimal()
...
```{r variance-decomposition, eval = F}

# model uncertainty: one value for MES, 1000 values for coefs

mu_model <- matrix(NA_real_, nrow = nrow_theta, ncol = nrow_isere)
for(i in 1:nrow_theta){
   mu_model[i,] <- with(as.list(theta_poc[i,]), # listing the coefficients allows to have 2 named numbers to be used in the function
                      f(isere$mes_for_fit, a, b))
}

isere$poc_var_model <- apply(mu_model, 2, var)
var_model_mean <- mean(var_model)

# MES uncertainty: one value per coef, 1000 values for MES
thetas <- coef(pocfit)
mu_mes <-  f(mes_mat, thetas[1], thetas[2])

isere$poc_var_mes <- apply(mu_mes, 2, var)
var_mes_mean <- mean(var_mes)

ggplot(isere, aes(x = mes_for_fit))+
  geom_line(aes(y = poc_var_model, col = "model"), alpha = 0.5)+
  geom_line(aes(y = poc_var_mes, col = "mes"))+
  geom_line(aes(y = poc_var_mean, col = "mean variance"), linetype = "dashed")+
  labs(x = "MES, mg.L", y = "Variance", col = "")+
...
Pour des hautes valeurs de MES: la variance du modèle ne vient que du bruit, elle est proche de zéro sinon. Celle qui vient du modèle, celle qui vient des MES, et la variance moyenne du modèle. 



ça part en couille, on tourne en rond sur la meilleure manière d'intégrer la variance
...
ggplot(poc_isere, aes(x = mes_for_fit))+
  geom_point(aes(x=mes_for_fit, y = poc_prop, col = "measured", shape = "measured"))+ 
  geom_point(aes(x=mes_for_fit, y = poc_fitted_boot, col = "fitted", shape = "fitted"))+ 
  geom_ribbon(aes(x = mes_for_fit, ymin = poc_fitted_ci_low, ymax = poc_fitted_ci_high), fill = "gray80", alpha = 0.5)+
  geom_abline(slope = 0, intercept = aspt_value, col = "darkred")+
...
ggplot(poc_isere)+
   geom_point(aes(x=poc_prop, y = poc_nls_fitted), col = "darkblue")+ 
   geom_errorbar(aes(x=poc_prop, ymin = poc_fitted_ci_low, ymax = poc_fitted_ci_high), fill = "gray80", alpha = 0.5)+
...
#plot(pocfit)
#qqnorm(residuals(pocfit, type = "response"))
#qqline(residuals(pocfit, type = "response"))

```

#### Manual bootstrap

```{r manual-coef-boot-and-ci}

n_boot <- 1000
n_obs <- nrow(poc_isere)

coefs_boot_nls <- matrix(NA, nrow = n_boot, ncol = 2, dimnames = list(NULL, c("a","b")))
...
for(i in seq_len(n_boot)){
  ...
  tryCatch(
    {pocfit_bg <- nls(
      ...
      data = df_boot,
      start = c(a = 0.1, b = 0.5),)
    coefs_boot_nls[i,] <- coef(pocfit_bg)}, 
    error = function(e) NULL) 
}

coefs_boot_nls <- na.omit(coefs_boot_nls)
length(coefs_boot_nls)
apply(coefs_boot_nls, 2, quantile, probs = c(0.025, 0.975)) #ic coef

ggplot(as.data.frame(coefs_boot_nls))+
  geom_boxplot(aes(x = "a", y = a, col = "a"))+
  geom_boxplot(aes(x = "b", y = b, col = "b"))+
  labs(x="", y = "")+
  scale_color_manual(values = c("darkblue", "lightblue"))+
  theme_minimal()+
...
fitted_boot_nls <- apply(coefs_boot_nls, 1, function(p){p["a"] * mes_for_fit^p["b"] + aspt_value}) # les MC worlds sont en colonne

fitted_boot_nls_ic <- apply(fitted_boot_nls, 1, quantile,
                 probs = c(0.025, 0.5, 0.975), na.rm = TRUE)
fitted_boot_nls_mean <- apply(fitted_boot_nls, 1, mean)

poc_isere_boot_nls <- poc_isere %>% mutate(poc_fitted = fitted_boot_nls_mean, upr = fitted_boot_nls_ic[3, ], lwr = fitted_boot_nls_ic[1,])

ggplot(poc_isere_boot_nls)+
  geom_point(aes(x=mes_for_fit, y = poc_prop, col = "measured", shape = "measured"))+ 
...
ggplot(poc_isere_boot_nls, aes(x=poc_prop))+
...
fitted_wboot_nls_ic <- apply(fitted_boot_nls, 1, quantile,
                 probs = c(0.025, 0.5, 0.975), na.rm = TRUE)
...
plot(pocfitg)
#qqnorm(residuals(pocfitg, type = "normalized"))
#qqline(residuals(pocfitg, type = "normalized"))
```

#### Approximation linéaire . DEPRECATED

```{r intervalles-de-confiance-deta-method}
beta <- coef(pocfitg) # get a and b
V_beta <- vcov(pocfitg) # matrice de covariance

a <- beta["a"]
b <- beta["b"]

# paramètre de variance - puissance de la variance var ~ y^theta
theta <- as.numeric(
  coef(pocfitg$modelStruct$varStruct, unconstrained = F)
)
sigma <- pocfitg$sigma

df_resid <- pocfitg$dims$N - pocfitg$dims$p
t_crit <- qt(0.975, df = df_resid)

mes_for_fit <- poc_isere$mes_for_fit
poc_fitted <- a*mes_for_fit^b + aspt_value

G <- cbind(mes_for_fit^b, a*mes_for_fit^b * log(mes_for_fit)) # gradient analytique df/dtheta

var_fit <- rowSums((G %*% V_beta) * G)
se_fit <- sqrt(var_fit)

poc_isere_delta <- poc_isere %>% mutate(poc_fitted = poc_fitted, lwr = poc_fitted - t_crit * se_fit, upr = poc_fitted + t_crit*se_fit)

formula_pocfitg <- paste("GNLS :", round(a, 3),"x MES ^", round(b, 3), "+", round(aspt_value, 3))

ggplot(poc_isere_delta)+
  geom_point(aes(x=mes_for_fit, y = poc_prop, col = "measured", shape = "measured"))+ 
  geom_point(aes(x=mes_for_fit, y = poc_fitted, col = "fitted", shape = "fitted"))+ 
  geom_ribbon(aes(x = mes_for_fit, ymin = lwr, ymax = upr), fill = "gray80", alpha = 0.5)+
  geom_abline(slope = 0, intercept = aspt_value, col = "darkred")+
...
ggplot(poc_isere_delta, aes(x=poc_prop))+
  geom_point(aes(y=poc_fitted), col = "darkblue")+ 
  geom_errorbar(aes(ymin = lwr, ymax = upr), fill = "gray80", alpha = 0.5)+
  geom_abline(slope = 1, intercept = 0, col = "darkred")+
  annotate(geom = "label", label = formula_pocfitg, x = Inf, y = Inf, hjust = 1.1, vjust = 1.1)+
...
```{r pred-isere}
isere$mes_for_fit <- isere$cmes_g.L * 1e3 
with(isere, sum(mes_for_fit == 0 | is.na(mes_for_fit), na.rm = T)) # 144 -> removed from df

isere <- isere %>% filter(mes_for_fit == 0 | is.na(mes_for_fit))

isere$poc_prop <- isere$poc_percent / 100
isere$aspt <- aspt_value

nrow_isere <- nrow(isere)
nrow_boot <- nrow(coefs_boot)

# predict POC pour mes > 0
isere$poc_fitted <- predict(pocfitg, newdata = isere, na.action = na.pass) 
...
mes_mat <- matrix(NA_real_, nrow = nrow_boot, ncol = nrow_isere)

for(i in 1:nrow(isere)){
  mes_mat[,i] <-rnorm(nrow_boot, mean = isere$mes_for_fit[i], sd = mes_u) # chaque colonne de la matrice correspond à une observation de MES
}
...
for(i in 1:nrow_boot){
   mu_nls[i,] <- with(as.list(coefs_boot[i,]), # listing the coefficients allows to have 2 named numbers to be used in the function
                      f(mes_mat[i, ], a, b))
}
...
noise_mat <- matrix(rnorm(nrow_boot*nrow_isere, mean = 0, sd = 1), nrow = nrow_boot, ncol = nrow_isere)
noise_mat <- noise_mat*sigma_mat

pred_nls <- mu_nls + noise_mat
...
boot_pred <- t(
  apply(pred_nls, 2, mean, na.rm = T)
) 

## insert in isere

isere <- isere %>% mutate(poc_boot_pred = poc_boot_pred,
                          poc_pred_lower = pi[,1],
                          poc_pred_upper = pi[,2])
...
ggplot(isere, aes(x = mes_for_fit))+
  ...
  geom_point(aes(y = poc_fitted, col = "fitted with all events"), alpha = 0.5)+
  geom_point(aes(y = poc_boot_pred, col ="poc_boot_pred"), alpha = 0.5)+
  labs(x = "MES, mg.L", y = "POC, %", col = "")+
...
```{r diagnostiques-erreurs}
# 1. Combien de prédictions > 1 ?
sum(isere$poc_fitted > 1, na.rm = TRUE)
sum(isere$poc_pred_upper > 1, na.rm = TRUE)
mean(isere$poc_fitted > 1, na.rm = TRUE) * 100  # pourcentage

sum(isere$poc_boot_pred > 1, na.rm = TRUE)
mean(isere$poc_boot_pred > 1, na.rm = TRUE) * 100  # pourcentage

# Combien de mes_for_fit == 0 ?
sum(isere$mes_for_fit == 0, na.rm = TRUE)

# Prédiction à x = 0 avec vos paramètres
a <- coef(pocfitg)["a"]
b <- coef(pocfitg)["b"]

cat("b =", b, "\n")
cat("0^b =", 0^b, "\n")
cat("Prédiction à x=0 :", a * 0^b + aspt_value, "\n")

# Vérifier les prédictions aux zéros
pred_at_zero <- isere %>%
  filter(mes_for_fit == 0) %>%
  select(mes_for_fit, poc_fitted, poc_pred_lower, poc_pred_upper)
...
On utilise les 1000 itérations du bootsraping effectué lors de la création des intervalles de prédiction du modèle, stockés dans pred_nls.
