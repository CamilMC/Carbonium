...
ggplot(poc_isere)+
  ...
  annotate(geom = "label", label = formula_pocfit, x = Inf, y = 5, hjust = 1.1, vjust = 1.1)+
...
```{r variance-model}
pocfit2 <- nlme::nlme(poc_percent ~ a*mes_for_fit^b+aspt,
                      data = poc_isere,
                      start = list(a = 1, b = -0.5),
                          fixed = a+b ~ 1, 
                          weights = nlme::varPower(form = ~ mes_for_fit ^ b),
...
######## ADD THIS IN THE NEXT STEPSSSS ##### SEE MY DEAR FRIEND CHATGPT
...
## adds noise
pred_nls <- mu_nls + matrix(rnorm(n = nrow_theta*nrow_isere, mean = 0, sd = sigma_poc), nrow = nrow_theta) #sigma is the standard deviation of the random error term

isere$poc_var_tot <- apply(pred_nls, 2, var) # total variance, for prediction uncertainty
...
## Test: annual flux for POC fitted from asymptot vs annual flux for POC fitted with powerlaw
